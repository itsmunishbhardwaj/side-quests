# Needle — Claude Project Setup (one-time, ~10 min)
...
1. Create a folder named **`Needle`** in My Drive root.
...
Open a new chat inside the Needle Project and paste the contents of `claude-project/kickoff-prompt.md`. Claude will:

- Verify it can read all 5 Drive files
- Verify Calendar read access by listing tomorrow's events
- Verify it can find the `Needle` calendar for writing events
...
- The Needle Project appears under **Projects**. Tap to switch.
- iOS Share Sheet → Claude lets you send a photo straight into a Needle chat from anywhere.
...
```
Adds color-coded events to your `Needle` calendar. Re-running clears old events first.
...
- **"Claude can't find the Needle calendar."** In Google Calendar, confirm the calendar exists and is visible in your primary calendar list. If still stuck, tell Claude its calendar ID explicitly (find it in Google Calendar Settings → calendar name → "Integrate calendar").
